...
const TicketListPage = ({ title, description, showToggle = false, viewType = 'default' }) => {
    ...
    return (
        <div className="inquiry-page-surface relative flex h-full min-h-0 flex-col overflow-hidden overflow-x-hidden p-4" dir="rtl">
            ...
            <div className={`relative z-40 mb-3 shrink-0 flex-nowrap items-center justify-between gap-3 overflow-x-auto pb-1 ${organization.filtersAvailable ? 'flex' : 'hidden'}`}>
                <label className="inquiry-control flex h-9 min-w-[220px] flex-1 items-center rounded-xl px-3 shadow-[0_4px_12px_rgba(37,99,235,0.08)] transition focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-400/20 lg:max-w-[500px]">
                    <input
                        value={searchQuery}
                        maxLength={100}
