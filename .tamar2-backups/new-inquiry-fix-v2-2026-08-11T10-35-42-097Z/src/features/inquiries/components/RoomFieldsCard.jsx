    ...
    return (
        <section className="inquiry-panel flex h-full min-h-0 basis-[64%] flex-col overflow-hidden rounded-2xl p-4" dir="rtl">
            <div className="flex h-11 shrink-0 items-center gap-2 border-b border-[var(--color-border)] pb-3">
                ...
                <CompactSelect className="w-[clamp(120px,11vw,150px)] shrink-0" prefix={LABEL_ENVIRONMENT} value={environmentId} onChange={(event) => setEnvironmentId(event.target.value)} options={environments} />
                <CompactSelect className="w-[clamp(120px,11vw,150px)] shrink-0" prefix={LABEL_SUB_ENVIRONMENT} value={subEnvironmentId} onChange={(event) => setSubEnvironmentId(event.target.value)} options={selectedEnvironment.subEnvironments} disabled={!selectedEnvironment.subEnvironments.length} />
                <CompactSelect className="w-[clamp(120px,11vw,150px)] shrink-0" prefix={LABEL_ROOM} value={roomId} onChange={(event) => setRoomId(event.target.value)} options={selectedSubEnvironment?.rooms ?? []} disabled={!selectedSubEnvironment?.rooms?.length} />
