...
const categoryInput = (input, { requireName = false } = {}) => {
    const output = {};
    if (Object.hasOwn(input, 'name')) Object.assign(output, normalizeCategoryName(input.name));
    else if (requireName) throw boardError(400, 'INVALID_CATEGORY_NAME', 'Category name is required');
    if (Object.hasOwn(input, 'description')) output.description = input.description;
...
class TicketBoardCategoryService {
    ...
    async context(actorId, roomId, boardType) {
        return this.authorizationService.authorize(actorId, roomId, boardType);
    }
    ...
    async create(actorId, roomId, boardType, input) {
        const { lineage } = await this.context(actorId, roomId, boardType);
        ...
        this.realtimePublisher.publishCategory('board:category-created', category);
        return toCategoryDto(category);
    }

    async update(actorId, roomId, boardType, categoryId, expectedVersion, input) {
        await this.context(actorId, roomId, boardType);
        const values = categoryInput(input);
        const current = await this.categoryRepository.findScoped(categoryId, roomId, boardType);
        if (!current || !current.isActive) throw categoryNotFound();
        if (current.version !== expectedVersion) {
            throw boardError(409, 'BOARD_CATEGORY_VERSION_CONFLICT', 'Board category version is stale');
        }
        const changed = Object.entries(values).some(([key, value]) => !equalValue(current[key], value));
        if (!changed) throw boardError(400, 'EMPTY_CATEGORY_UPDATE', 'Category update must contain an effective change');
        const category = await this.categoryRepository.updateActive(
            categoryId, roomId, boardType, expectedVersion, { ...values, updatedBy: actorId }
        );
        if (!category) throw boardError(409, 'BOARD_CATEGORY_VERSION_CONFLICT', 'Board category version is stale');
        this.realtimePublisher.publishCategory('board:category-updated', category);
        return toCategoryDto(category);
    }

    async archive(actorId, roomId, boardType, categoryId, expectedVersion) {
        await this.context(actorId, roomId, boardType);
        const current = await this.categoryRepository.findScoped(categoryId, roomId, boardType);
        if (!current || !current.isActive) throw categoryNotFound();
        if (current.version !== expectedVersion) {
        ...
        const category = await this.categoryRepository.archiveActive(
            categoryId, roomId, boardType, expectedVersion, actorId, new Date()
        );
        if (!category) throw boardError(409, 'BOARD_CATEGORY_VERSION_CONFLICT', 'Board category version is stale');
        this.realtimePublisher.publishCategory('board:category-archived', category);
        return toCategoryDto(category);
    }
}
