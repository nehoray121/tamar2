...
const TICKET_SELECTION_TOOLBAR_EXIT_ID = 'ticket-selection-toolbar-exit';
const TICKET_SELECTION_TOOLBAR_STYLE_ID = 'ticket-selection-toolbar-style';
const TICKET_SELECTION_FILTER_TEXTS = [
    'כל הפניות',
    'פניות נעוצות',
    'פניות לא נעוצות',
    'ללא נעוצות',
...
const getTextContent = (element) => (element?.textContent || '').replace(/\s+/gu, ' ').trim();

const ensureSelectionToolbarStyle = () => {
    if (typeof document === 'undefined') return;
    if (document.getElementById(TICKET_SELECTION_TOOLBAR_STYLE_ID)) return;

    const style = document.createElement('style');
    style.id = TICKET_SELECTION_TOOLBAR_STYLE_ID;
    style.textContent = [`
        [data-selection-filter-hidden=\"true\"] {`,
        `    display: none !important;`,
        `}`,
        ``,
        `#${TICKET_SELECTION_TOOLBAR_EXIT_ID} {`,
        `    display: inline-flex;`,
        `    align-items: center;`,
        ...
        `}`,
        ``,
        `#${TICKET_SELECTION_TOOLBAR_EXIT_ID}:hover {`,
        `    background: color-mix(in srgb, var(--color-surface-raised) 84%, var(--color-primary) 16%);`,
        `}`,
        ``,
        `#${TICKET_SELECTION_TOOLBAR_EXIT_ID} .selection-exit-icon {`,
        `    display: inline-flex;`,
        `    align-items: center;`,
        `    justify-content: center;`,
        `    width: 18px;`,
...
const findSelectionToolbarHost = () => {
    if (typeof document === 'undefined') return null;

    const searchInput = document.querySelector('input[placeholder*=\"חיפוש לפי מספר פנייה\"], input[placeholder*=\"חיפוש\"]');
    ...
    while (node && node !== document.body) {
        ...
        if (buttons >= 2 && inputs >= 1) return node;
...
const findSelectionExitTarget = () => {
    if (typeof document === 'undefined') return null;

    return Array.from(document.querySelectorAll('button')).find((button) => {
        const text = getTextContent(button);
        return /סיום|ביטול/iu.test(text) && button.closest('.inquiry-selection-bar');
    }) || null;
};

const applySelectionModeToolbarCleanup = (active) => {
    if (typeof document === 'undefined') return;

    ensureSelectionToolbarStyle();

    const allButtons = Array.from(document.querySelectorAll('button'));
    for (const button of allButtons) {
        const text = getTextContent(button);
        const shouldHide = active
            && TICKET_SELECTION_FILTER_TEXTS.some((label) => text.includes(label))
            && !text.includes('מצב בחירה')
            && !text.includes('בחר הכל בעמוד')
            && !text.includes('בחר את כל')
            && !text.includes('סיום');

        if (shouldHide) {
            button.setAttribute('data-selection-filter-hidden', 'true');
            continue;
        }

        button.removeAttribute('data-selection-filter-hidden');
    }

    const host = findSelectionToolbarHost();
    let exitButton = document.getElementById(TICKET_SELECTION_TOOLBAR_EXIT_ID);

    if (active && host) {
        if (!exitButton) {
            exitButton = document.createElement('button');
            exitButton.id = TICKET_SELECTION_TOOLBAR_EXIT_ID;
            exitButton.type = 'button';
            exitButton.innerHTML = '<span class=\"selection-exit-icon\">×</span><span>יציאה</span>';
            exitButton.addEventListener('click', () => {
                findSelectionExitTarget()?.click();
            });
        }

        if (!host.contains(exitButton)) host.appendChild(exitButton);
    } else if (exitButton) {
        exitButton.remove();
    }
};
const TicketListPage = ({ title, description, showToggle = false, viewType = 'default' }) => {
    ...
    useEffect(() => {
        ...
        let frame = 0;
        const apply = () => applySelectionModeToolbarCleanup(organization.selectionMode);
        ...
        return () => {
            observer.disconnect();
            cancelAnimationFrame(frame);
            applySelectionModeToolbarCleanup(false);
        };
    ...
    return (
        <div className="inquiry-page-surface relative flex h-full min-h-0 flex-col overflow-hidden overflow-x-hidden p-4" dir="rtl">
            {(sortDropdownOpen || priorityDropdownOpen || (showPinFilter && pinDropdownOpen)) && <div className="fixed inset-0 z-30" onClick={closeAllDropdowns} />}
