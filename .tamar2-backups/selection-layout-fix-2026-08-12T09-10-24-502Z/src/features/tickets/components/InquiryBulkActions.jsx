...
const actionButton = 'inquiry-control flex h-9 items-center gap-2 whitespace-nowrap rounded-xl px-3 text-[12px] font-black shadow-sm transition focus:outline-none focus:ring-2 focus:ring-blue-400/30';
const activeActionButton = 'border-blue-500 bg-blue-500/12 text-[var(--color-primary)] ring-1 ring-inset ring-blue-500/35';
...
const InquiryBulkActions = ({
    ...
    return (
        <>
            <div className="flex flex-wrap items-center gap-1.5 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)]/55 p-1">
                <button
                    data-testid="board-bulk-toggle"
                    type="button"
                    onClick={active ? onCancel : onStart}
                    className={`flex h-9 items-center gap-2 whitespace-nowrap rounded-xl px-3.5 text-[12px] font-black transition focus:outline-none focus:ring-2 focus:ring-blue-400/30 ${
                        active
                ...
                >
                    <Icon
                        name={active ? 'close' : 'check'}
                        className="h-3.5 w-3.5"
                    />
                    {active ? 'סיום בחירה' : 'בחירת פניות'}
                </button>

                {active && hasActiveFilters && (
                    <span
                        className="inline-flex h-9 items-center gap-2 rounded-xl border border-amber-400/30 bg-amber-500/10 px-3 text-[11px] font-black text-amber-700 dark:text-amber-200"
                        title="הבחירה חלה רק על התוצאות שתואמות לחיפוש ולסינונים הנוכחיים"
                    >
                        <Icon name="filter" className="h-3.5 w-3.5" />
                        סינון פעיל
                        <span className="hidden text-[10px] font-semibold opacity-80 xl:inline">
                            הבחירה חלה על התוצאות המסוננות
                        </span>
                    </span>
                )}

                {active && currentPageCount > 0 && (
                    <button
                        type="button"
