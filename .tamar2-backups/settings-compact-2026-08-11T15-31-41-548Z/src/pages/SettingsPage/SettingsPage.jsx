...
const tabs = [
    ...
    { id: 'assignment', label: 'שיוך פנייה' },
...
const getScopeLabel = (scope) => ({ system: 'כללי', environment: 'סביבה', subEnvironment: 'תת-סביבה', room: 'חדר' }[scope] || 'חדר');
const getGroupLabel = (group) => group === 'system' ? 'שדות קבועים' : 'שדות חדר';
...
const TemplatePanel = ({ onCreate }) => (
    <SectionShell title="בחרו תבנית" className="h-full max-h-[560px]">
        <div dir="rtl" className="space-y-1.5 overflow-y-auto p-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
    ...
    return (
        ...
        >
            ...
            <span className="min-w-0">
                ...
                <span className="mt-1 flex min-w-0 items-center gap-1.5 truncate text-[11px] font-bold text-[var(--color-text-muted)]">
                    {field.required
                        ? <span className="shrink-0 text-red-600">חובה</span>
                        : <span className="shrink-0">רשות</span>}
                    <span className="shrink-0 opacity-50">•</span>
                    <span className="truncate">{typeLabel}</span>
                    {parentField && (
                        <>
                            <span className="shrink-0 opacity-50">•</span>
                            <span className="truncate">({'תלוי ב: ' + parentField.name})</span>
                        </>
                    )}
                </span>
            </span>
...
const AssignmentSettings = ({ settings, setSettings }) => {
    const update = (key, value) => setSettings((current) => ({ ...current, [key]: value }));

    return (
        <div className="mx-auto h-full max-w-[980px] overflow-y-auto px-1 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <SectionShell
                title="שיוך פנייה"
                meta="הגדרות שיוך מטפלים בחדר הנוכחי."
                className="overflow-visible"
            >
                <div className="grid gap-3 p-4 md:grid-cols-2">
                    <ToggleSwitch
                        checked={settings.userAssignmentEnabled !== false}
                        onChange={(value) => update('userAssignmentEnabled', value)}
                        label="הפעלת שיוך משתמשים"
                        description="מאפשר לבחור ולשנות מטפל באופן ידני מתוך משתמשי החדר."
                    />
                    <ToggleSwitch
                        checked={Boolean(settings.automaticAssignmentEnabled)}
                        onChange={(value) => update('automaticAssignmentEnabled', value)}
                        label="שיוך אישי אוטומטי"
                        description="כאשר פעיל, פנייה חדשה יכולה לקבל מטפל זמין באופן אוטומטי."
                    />
                </div>
                <div className="mx-4 mb-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-4 py-3 text-[12px] font-semibold leading-6 text-[var(--color-text-secondary)]">
                    ההגדרות נשמרות אוטומטית עבור החדר הנוכחי ומשפיעות על אפשרויות השיוך בפניות.
                </div>
            </SectionShell>
        </div>
    );
