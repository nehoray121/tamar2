...
const toolbarButton = 'inquiry-control flex h-9 shrink-0 items-center gap-2 whitespace-nowrap rounded-xl px-3 text-[12px] font-black shadow-sm transition focus:outline-none focus:ring-2 focus:ring-blue-400/30';
const activeButton = 'border-blue-500 bg-blue-500/12 text-[var(--color-primary)] ring-1 ring-inset ring-blue-500/35';
...
const InquiryBulkActions = ({
    ...
    if (!active) {
        return (
            ...
            >
                <Icon name="check" className="h-3.5 w-3.5 text-[var(--color-primary)]" />
                בחירת פניות
            </button>
        );
    }
    ...
    return (
        <>
            <div className="flex min-w-max shrink-0 items-center gap-1.5 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)]/55 p-1">
                <button
                    data-testid="board-bulk-toggle"
                    type="button"
                    onClick={onCancel}
                    className="flex h-9 shrink-0 items-center gap-2 whitespace-nowrap rounded-xl bg-blue-600 px-3.5 text-[12px] font-black text-white shadow-sm transition focus:outline-none focus:ring-2 focus:ring-blue-400/30"
                >
                ...
                {currentPageCount > 0 && (
                    <button
                        ...
                        className={`${toolbarButton} ${currentPageSelected ? activeButton : ''} disabled:cursor-not-allowed disabled:opacity-50`}
                    >
                        <Icon
                        ...
                        />
                        {currentPageSelected
                            ? `בטל בחירת ${currentPageCount} בעמוד`
                            : 'בחר הכל בעמוד'}
                    </button>
                )}
                ...
                {totalMatchingCount > 0 && (
                    <button
                        type="button"
                        onClick={allMatchingSelected ? onClearSelected : onSelectAllMatching}
                        disabled={selectingAll || running}
                        className={`${toolbarButton} ${allMatchingSelected ? activeButton : ''} disabled:cursor-not-allowed disabled:opacity-50`}
                    >
                        {selectingAll ? (
                            <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-current border-t-transparent" aria-hidden="true" />
                        ) : (
                            <Icon
                                name={allMatchingSelected ? 'close' : 'dashboard'}
                                className="h-3.5 w-3.5 text-[var(--color-primary)]"
                            />
                        )}
                        {selectingAll
                            ? hasActiveFilters
                                ...
                                : 'בוחר את כל הפניות...'
                            : allMatchingSelected
                                ? `בטל — ${allSelectedLabel}`
                                : allMatchingLabel}
                    </button>
                )}

                <span className={`inline-flex h-9 min-w-[74px] shrink-0 items-center justify-center rounded-xl border px-3 text-[11px] font-black ${
                    selectedCount
            ...
            {selectedCount > 0 && (
                <div className="inquiry-selection-bar fixed bottom-8 left-1/2 z-[70] flex -translate-x-1/2 items-center gap-3 rounded-2xl px-4 py-2" dir="rtl">
                        ...
                        {progress && (
                            <div className="text-[10px] font-bold inquiry-muted-text">
                                {running
                                    ...
                                    : `${progress.succeeded} הצליחו, ${progress.failed} נכשלו${progress.conflicts ? `, ${progress.conflicts} התנגשויות` : ''}`}
                            </div>
                        )}
                    </div>
                    ...
                    <button type="button" onClick={onCancel} disabled={running} className="flex h-9 items-center whitespace-nowrap rounded-xl px-2.5 text-[12px] font-black inquiry-muted-text transition hover:bg-[var(--color-surface-muted)] disabled:opacity-50">
                        סיום
                    </button>
                </div>
            )}
