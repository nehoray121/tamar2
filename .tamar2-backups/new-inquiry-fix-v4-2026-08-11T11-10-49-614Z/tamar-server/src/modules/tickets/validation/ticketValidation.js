...
const parseReserveTicketNumber = (request, _response, next) => {
    try {
        assertExactKeys(request.body, ['roomId']);
        request.ticketReservationInput = {
...
const parseCreateTicket = (request, _response, next) => {
    try {
        ...
        assertExactKeys(request.body, ['roomId', 'subject', 'description', 'priority', 'fieldValues', 'reservationId']);
        request.ticketInput = {
            roomId: requireObjectId(request.body.roomId, 'roomId'),
            reservationId: optionalObjectId(request.body.reservationId, 'reservationId'),
