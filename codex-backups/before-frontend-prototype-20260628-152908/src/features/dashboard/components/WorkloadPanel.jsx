...
import SectionExpandButton from './SectionExpandButton.jsx';
        ...
        const WorkloadPanel = ({ rows, expanded, onToggle }) => {
            ...
            return (
                <DashboardCard className={`dashboard-card-motion flex h-full min-h-0 flex-col p-4 ${expanded ? 'dashboard-expanded-card' : ''}`} dir="rtl">
                    <div className="mb-2 flex shrink-0 items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                            <Icon name="user" className="h-[18px] w-[18px] text-blue-500" />
                    ...
                    <div className={`min-h-0 flex-1 ${expanded ? 'overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]' : 'overflow-hidden'}`}>
                        <div className={`${expanded ? 'dashboard-legend-reveal' : ''} space-y-3 pr-1`}>
                            {visibleRows.map(row => {
                                ...
                                return (
                                    <div key={row.name}>
                                        <div className="mb-1 flex items-end justify-between gap-3">
                                            ...
                                            <span className="text-xs font-black text-slate-900">{row.total}</span>
                                        </div>
