...
import Icon from '../../../components/common/Icon.jsx';
        ...
        const DashboardKpiCard = ({ title, subtitle, value, icon, accent, mode = 'dashboard', actionIcon, actionLabel, onAction, isActionDisabled = false }) => {
            ...
            return (
                <article className={`group relative flex h-full flex-col rounded-[24px] border bg-white text-right ${palette.border} ${palette.shadow} ${isModal ? 'min-h-[168px] px-4 pb-4 pt-5' : 'h-[clamp(172px,18vh,178px)] min-h-[172px] px-4 pb-3 pt-3'} ${!isModal ? 'dashboard-card-motion' : ''}`}>
                    {onAction && (
                        <button
                            type="button"
                            onClick={onAction}
                    ...
                    <div className="flex items-start justify-between gap-2.5">
                        <div className="min-w-0 flex-1 pr-1">
                            <h4 className={`font-black text-slate-900 ${isModal ? 'text-[10px] leading-5' : 'text-[14px] leading-5'}`}>{title}</h4>
                    ...
                    <div className={`mt-auto ${isModal ? 'pt-5' : 'pt-2 pb-6'}`}>
                        <div className={`font-black tracking-tight ${palette.value} ${isModal ? 'text-[44px] leading-none' : 'text-[38px] leading-none'}`}>{value}</div>
