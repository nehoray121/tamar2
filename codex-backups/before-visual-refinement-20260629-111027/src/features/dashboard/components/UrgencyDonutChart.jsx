...
const UrgencyDonutChart = ({ data, onSegmentClick, isExpanded = false, hideCenter = false }) => {
    ...
    const chartSize = isExpanded ? 204 : 184;
    const labelPadding = isExpanded ? 76 : 38;
    const size = chartSize + (labelPadding * 2);
    const strokeWidth = isExpanded ? 32 : 28;
    const radius = (chartSize - strokeWidth) / 2;
    const total = data.reduce((sum, item) => sum + item.value, 0);
    const centerRadius = Math.max(0, radius - (strokeWidth / 2) - 7);
    const center = size / 2;
    const chartOffsetY = isExpanded ? 0 : -4;
    const labelInset = isExpanded ? 26 : 18;
    let currentAngle = 0;
    ...
    return (
        <div className="relative flex items-center justify-center p-1.5">
            <div className={`pointer-events-none absolute rounded-full bg-gradient-to-b from-blue-50 to-white shadow-inner ${isExpanded ? 'h-48 w-48' : 'h-40 w-40'}`} style={{ transform: `translateY(${chartOffsetY}px)` }} />
            <svg width={size} height={size} className="relative z-10 overflow-visible drop-shadow-sm" style={{ transform: `translateY(${chartOffsetY}px)` }}>
                ...
                {data.map(item => {
                    ...
                    const connectorStart = pointOnCircle(labelAngle, outerRingRadius + (isExpanded ? 12 : 7));
                    const connectorBend = pointOnCircle(labelAngle, outerRingRadius + (isExpanded ? 42 : 24));
                    const isRightSide = connectorBend.x >= center;
                    const connectorEndX = Math.min(size - labelInset, Math.max(labelInset, connectorBend.x + (isRightSide ? (isExpanded ? 44 : 24) : (isExpanded ? -44 : -24))));
                    const labelPoint = {
                        x: Math.min(size - labelInset, Math.max(labelInset, connectorEndX + (isRightSide ? (isExpanded ? 14 : 8) : (isExpanded ? -14 : -8)))),
                        y: Math.min(size - (isExpanded ? 28 : 18), Math.max(isExpanded ? 28 : 18, connectorBend.y))
                    };
            ...
            {!hideCenter && (
                <div className={`donut-center absolute z-20 flex cursor-default flex-col items-center justify-center rounded-full bg-white/80 shadow-[0_18px_45px_rgba(37,99,235,0.12)] backdrop-blur-sm ${isExpanded ? 'px-6 py-5' : 'px-4 py-3.5'}`} style={{ transform: `translateY(${chartOffsetY}px)` }}>
                    <span className={`font-black text-slate-900 ${isExpanded ? 'text-4xl' : 'text-3xl'}`}>{total}</span>
                    <span className={`font-bold text-slate-500 ${isExpanded ? 'text-sm' : 'text-xs'}`}>סה״כ פניות</span>
                </div>
            )}
        </div>
    );
