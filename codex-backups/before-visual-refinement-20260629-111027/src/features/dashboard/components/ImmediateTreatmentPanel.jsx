                    ...
                    <div className={`min-h-0 flex-1 px-3 py-2 ${expanded ? 'overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]' : 'overflow-hidden'}`}>
                        <div className={`${expanded ? 'dashboard-legend-reveal' : ''} space-y-2`}>
                            {visibleItems.map((item, index) => (
                                <div key={item.id} className="flex items-center gap-2 rounded-2xl border border-transparent px-2.5 py-2 transition hover:border-slate-100 hover:bg-slate-50">
