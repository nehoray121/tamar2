...
const toolbarButton = 'flex h-9 items-center gap-2 whitespace-nowrap rounded-lg border border-[#C9E1FF] bg-white px-3 text-[12px] font-black text-slate-700 shadow-[0_3px_10px_rgba(37,99,235,0.08)] transition hover:border-[#93C5FD] hover:bg-[#EAF4FF] hover:text-[#3B82F6]';
...
const TicketListPage = ({ title, description, showToggle = false, viewType = 'default' }) => {
    ...
    return (
        <div className="relative flex h-full min-h-0 flex-col overflow-hidden overflow-x-hidden bg-[#EEF4FC] p-5" dir="rtl">
            {(searchDropdownOpen || sortDropdownOpen || priorityDropdownOpen) && <div className="fixed inset-0 z-30" onClick={closeAllDropdowns} />}

            <header className="relative z-10 mb-3 shrink-0">
            ...
            <div className="relative z-40 mb-3 flex shrink-0 flex-wrap items-center justify-between gap-3">
                <div className="flex h-9 min-w-[300px] flex-1 rounded-lg border border-[#C9E1FF] bg-white shadow-[0_3px_10px_rgba(37,99,235,0.08)] transition focus-within:border-[#3B82F6] focus-within:ring-2 focus-within:ring-blue-400/20 lg:max-w-[560px]">
                    <div className="relative border-l border-[#C9E1FF]">
                        <button onClick={() => { closeAllDropdowns(); setSearchDropdownOpen(!searchDropdownOpen); }} className="flex h-full items-center gap-2 rounded-r-lg bg-[#EAF4FF] px-3 text-[12px] font-black text-[#3B82F6]">
