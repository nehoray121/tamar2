...
import Icon from '../../../components/common/Icon.jsx';
        ...
        const DashboardKpiCard = ({ title, subtitle, value, icon, accent, mode = 'dashboard', actionIcon, actionLabel, onAction, isActionDisabled = false }) => {
            ...
            return (
                <article className={`group relative flex h-full flex-col rounded-[24px] border bg-white text-right ${palette.border} ${palette.shadow} ${isModal ? 'min-h-[168px] px-4 pb-4 pt-5' : 'h-[clamp(120px,14vh,138px)] min-h-[120px] px-4 pb-3 pt-3'} ${!isModal ? 'dashboard-card-motion' : ''}`}>
                    {onAction && (
                        <button
                            type="button"
                            onClick={onAction}
                            ...
                            className={`inline-flex items-center justify-center transition duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-300 disabled:cursor-not-allowed disabled:opacity-35 ${
                                isModal
                                    ...
                                    : `absolute bottom-3 left-1/2 h-6 min-w-[32px] -translate-x-1/2 rounded-xl border border-rose-200 bg-rose-50/95 px-2 text-rose-500 shadow-[0_6px_14px_rgba(244,63,94,0.14)] opacity-0 pointer-events-none translate-y-1 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100 hover:bg-rose-100 hover:text-rose-600`
                            }`}
                        >
                    ...
                    <div className={`mt-auto ${isModal ? 'pt-5' : 'pt-2.5 pb-6'}`}>
