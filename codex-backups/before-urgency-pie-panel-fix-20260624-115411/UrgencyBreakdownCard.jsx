                ...
                return (
                    <DashboardCard className="dashboard-card-motion dashboard-expanded-card flex h-full min-h-0 flex-col p-4" dir="rtl">
                        ...
                        <div className="flex min-h-0 flex-1 pt-3">
                            <div className="grid h-full min-h-0 w-full gap-4 overflow-hidden" style={{ gridTemplateRows: 'minmax(300px,340px) minmax(280px,1fr)' }}>
                                <div className="grid min-h-0 gap-4 overflow-hidden" style={{ gridTemplateColumns: 'minmax(360px,1.1fr) minmax(280px,0.72fr)' }}>
                                                    ...
                                                    {visibleDonutCategoryCards.map((category, index) => {
                                                        ...
                                                        return (
                                                            <article
                                                                ...
                                                                className="flex min-h-0 cursor-pointer flex-col rounded-[24px] border border-slate-200 bg-white p-3.5 text-right shadow-sm transition duration-300 hover:-translate-y-0.5 hover:shadow-md"
                                                            ...
                                                            >
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="mt-auto pt-3 text-right text-sm font-bold"
                                    ...
                                    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
                                        <div className="flex min-h-0 flex-1 items-center justify-center p-4">
                                            {showInlineDetails && selectedDonutCategory ? (

                                                <div className="flex h-full w-full flex-col rounded-[24px] border border-slate-100 bg-slate-50/70 p-5">
                                                    ...
                                                    <div className="flex items-start justify-between gap-4">
                                                        ...
                                                        <div className="text-right">
                                                            ...
                                                            <div className="flex items-center justify-end gap-2">
                                                                ...
                                                                <span className="text-lg font-black text-slate-950">{selectedDonutCategory.shortLabel}</span>

                                                                <span className="h-3 w-3 rounded-full" style={{ backgroundColor: selectedDonutCategory.color }} />

                                                            </div>

                                                            <p className="mt-1 text-sm font-semibold text-slate-400">פירוט הקטגוריה שנבחרה בתוך אזור הדחיפות</p>

                                                        </div>

                                                        <button type="button" onClick={() => setShowInlineDetails(false)} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-black text-slate-600 transition hover:border-slate-300">צמצם</button>

                                                    </div>

                                                    <div className="mt-5 grid grid-cols-3 gap-3">

                                                        <div className="rounded-3xl bg-white px-4 py-3 text-right shadow-sm ring-1 ring-slate-100">

                                                            <div className="text-xs font-black text-slate-400">פניות</div>

                                                            <div className="mt-2 text-3xl font-black text-slate-950">{selectedDonutCategory.value}</div>

                                                        </div>

                                                        <div className="rounded-3xl bg-white px-4 py-3 text-right shadow-sm ring-1 ring-slate-100">

                                                            <div className="text-xs font-black text-slate-400">אחוז מכלל הפניות</div>

                                                            <div className="mt-2 text-3xl font-black text-slate-950">{selectedDonutCategory.formattedPercentage}%</div>

                                                        </div>

                                                        <div className="rounded-3xl bg-white px-4 py-3 text-right shadow-sm ring-1 ring-slate-100">

                                                            <div className="text-xs font-black text-slate-400">רשומות מוצגות</div>

                                                            <div className="mt-2 text-3xl font-black text-slate-950">{visibleSelectedDonutInquiries.length}</div>
                                                    ...
                                                    <div className="mt-4 grid min-h-0 flex-1 gap-3">
                                                        ...
                                                        {visibleSelectedDonutInquiries.slice(0, 3).map(item => (

                                                            <div key={item.id} className="flex items-center justify-between rounded-3xl bg-white px-4 py-3 text-right shadow-sm ring-1 ring-slate-100">
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <span className="text-sm font-semibold text-slate-500">{formatDonutInquiryAge(item.date)}</span>

                                                                    <span className="inline-flex items-center gap-2 rounded-full bg-slate-50 px-2.5 py-1 text-xs font-black text-slate-600">
                                                        ...
                                                        {visibleSelectedDonutInquiries.length === 0 && (
                                                            ...
                                                            <div className="flex h-full items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white text-sm font-bold text-slate-400">אין פניות להצגה בקטגוריה זו</div>
                                ...
                                <section className="flex min-h-0 flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
                                    <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-3.5">
                                        ...
                                        <button type="button" onClick={() => selectedDonutCategory && setShowInlineDetails(current => !current)} disabled={!selectedDonutCategory} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-black text-slate-600 transition disabled:cursor-not-allowed disabled:opacity-40">{showInlineDetails ? 'צמצם' : 'לראות עוד'}</button>
                                    </div>
                                    ...
                                    <div className="min-h-0 flex-1 px-4 py-3">
                                        {visibleSelectedDonutInquiries.length === 0 ? (
                                            ...
                                            <div className="flex h-full min-h-[220px] flex-col overflow-hidden rounded-3xl border border-slate-100">
