...
const UrgencyDonutChart = ({ data, onSegmentClick, isExpanded = false, hideCenter = false }) => {
    ...
    const chartSize = isExpanded ? 190 : 154;
    const labelPadding = isExpanded ? 58 : 30;
    const size = chartSize + (labelPadding * 2);
    const strokeWidth = isExpanded ? 30 : 26;
    ...
    return (
        <div className="relative flex items-center justify-center p-1.5">
            <div className={`donut-halo pointer-events-none absolute rounded-full bg-gradient-to-b from-blue-50 to-white shadow-inner ${isExpanded ? 'h-44 w-44' : 'h-32 w-32'}`} style={{ transform: `translateY(${chartOffsetY}px)` }} />
            <svg width={size} height={size} className="relative z-10 overflow-visible drop-shadow-sm" style={{ transform: `translateY(${chartOffsetY}px)` }}>
                ...
                {data.map(item => {
                    ...
                    const connectorBend = pointOnCircle(labelAngle, outerRingRadius + (isExpanded ? 34 : 20));
                    const isRightSide = connectorBend.x >= center;
                    const connectorEndX = Math.min(size - labelInset, Math.max(labelInset, connectorBend.x + (isRightSide ? (isExpanded ? 34 : 18) : (isExpanded ? -34 : -18))));
                    const labelPoint = {
                        x: Math.min(size - labelInset, Math.max(labelInset, connectorEndX + (isRightSide ? (isExpanded ? 12 : 7) : (isExpanded ? -12 : -7)))),
            ...
            {!hideCenter && (
                <div className={`donut-center absolute z-20 flex cursor-default flex-col items-center justify-center rounded-full bg-white/80 shadow-[0_18px_45px_rgba(37,99,235,0.12)] backdrop-blur-sm ${isExpanded ? 'px-6 py-5' : 'px-4 py-3.5'}`} style={{ transform: `translateY(${chartOffsetY}px)` }}>
                    <span className={`font-black text-slate-900 ${isExpanded ? 'text-[34px]' : 'text-[28px]'}`}>{total}</span>
