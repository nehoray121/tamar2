...
const UrgencyBreakdownCard = ({
    ...
    const collapsedPanelHeight = totalDonutInquiryPages > 1 ? 284 : 244;
        ...
        return (
            <DashboardCard className="dashboard-card-motion flex h-full min-h-0 flex-col p-3" dir="rtl">
                ...
                <div className="flex min-h-0 flex-1 flex-col py-1">
                    <div className="flex min-h-[192px] flex-1 items-center justify-center">
                        <UrgencyDonutChart data={priorityData} onSegmentClick={handleDonutClick} />
                    </div>

                    <div className="mt-0 min-h-[38px] w-full shrink-0 rounded-2xl border border-slate-100 bg-slate-50/80 p-1">
                        <div className="flex items-start justify-center gap-1">
                            {visibleDonutCategories.map(item => (
                                <button key={item.label} type="button" title={item.label} onClick={() => handleDonutClick(item)} className="flex min-w-0 max-w-full items-center gap-1.5 rounded-xl bg-white px-2 py-0.5 text-[11px] font-black text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                                    <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: item.color }} />
                                    <span className="max-w-[76px] truncate" dir="rtl">{item.label}</span>
                                    <span className="rounded-md bg-slate-100 px-1.5 py-0.5 text-slate-500">{item.value}</span>
                                </button>
                            ))}
                            ...
                            {hasHiddenDonutCategories && (
                                    ...
                                    className="inline-flex h-[30px] shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white px-3 text-sm font-black text-slate-500 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:text-blue-700 hover:shadow-md"
                                >
                                    ...
                                </button>
                            )}
    ...
    return (
        <DashboardCard className="dashboard-card-motion dashboard-expanded-card flex h-full min-h-0 flex-col p-4" dir="rtl">
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-3 pt-2.5">
                <div className="grid shrink-0 gap-3 xl:grid-cols-[minmax(340px,0.9fr)_minmax(0,1.15fr)]">
                    <section className="flex min-h-[224px] flex-col rounded-[26px] border border-slate-200 bg-white px-4 py-3 shadow-sm">
                    ...
                    <section className="flex min-h-[224px] flex-col rounded-[26px] border border-slate-200 bg-white shadow-sm">
                        ...
                        <div className="flex min-h-0 flex-1 items-start px-3 py-3">
                            {donutCategories.length === 0 ? (
                                ...
                                <div className={`grid w-full gap-2.5 ${categoryGridClass} ${categoryRowsClass}`}>
                                    {visibleDonutCategoryCards.map((category, index) => {
                                        ...
                                        return (
                                            <article
                                                ...
                                                className="flex min-h-[152px] cursor-pointer flex-col rounded-[22px] border border-slate-200 bg-white p-3 text-right shadow-sm transition duration-300 hover:-translate-y-0.5 hover:shadow-md"
                                            ...
                                            >
                                                ...
                                                <div className="mt-2.5 flex items-end justify-between gap-3">
                                                    ...
                                                    <div className="text-2xl font-black text-slate-950">{category.value}</div>
                                                </div>
                                                ...
                                                <button
                                                    ...
                                                    className="mt-auto pt-2 text-right text-[13px] font-bold"
