...
import Icon from '../../../components/common/Icon.jsx';
        ...
        const DashboardKpiCard = ({ title, subtitle, value, icon, accent, mode = 'dashboard', actionIcon, actionLabel, onAction, isActionDisabled = false }) => {
            ...
            return (
                <article className={`group relative flex h-full flex-col rounded-[24px] border bg-white text-right ${palette.border} ${palette.shadow} ${isModal ? 'min-h-[168px] px-4 pb-4 pt-5' : 'h-[clamp(148px,16vh,156px)] min-h-[148px] px-3.5 pb-2.5 pt-2.5'} ${!isModal ? 'dashboard-card-motion' : ''}`}>
                    {onAction && (
                        <button
                            type="button"
                            onClick={onAction}
                    ...
                    <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1 pr-1">
                            <h4 className={`font-black text-slate-900 ${isModal ? 'text-[10px] leading-5' : 'text-[13px] leading-[18px]'}`}>{title}</h4>
                            {subtitle && <p className={`mt-0.5 text-slate-400 ${isModal ? 'text-[10px] leading-4' : 'text-[10px] leading-[14px]'}`}>{subtitle}</p>}
                        </div>
                        <div className={`flex shrink-0 items-center justify-center border ${palette.iconBg} ${palette.iconBorder} ${isModal ? 'h-12 w-12 rounded-[18px]' : 'h-7 w-7 rounded-[13px]'}`}>
                            <Icon name={icon} className={`${isModal ? 'h-5 w-5' : 'h-3 w-3'} ${palette.icon}`} />
                        </div>
                    </div>

                    <div className={`mt-auto ${isModal ? 'pt-5' : 'pt-1.5 pb-5'}`}>
                        <div className={`font-black tracking-tight ${palette.value} ${isModal ? 'text-[44px] leading-none' : 'text-[32px] leading-none'}`}>{value}</div>
