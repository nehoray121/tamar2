...
const DashboardPage = () => {
    ...
    const handleUrgentInspect = (item) => {
        ...
        setModalConfig({
            isOpen: true,
            title: `׳₪׳ ׳™׳™׳” ׳׳˜׳™׳₪׳•׳: ${item.id}`,
            subtitle: `${item.requester} ײ²ֲ· ${item.priority}`,
            filteredData: [item]
        });
    };
            ...
            <main className="flex min-h-0 flex-1 flex-col overflow-hidden">
                ...
                <div className="min-h-0 flex-1 overflow-hidden">
                    {expandedSection === 'barChart' ? (
                        ...
                        <div dir="ltr" className="dashboard-motion grid h-full min-h-0 grid-cols-12 grid-rows-[minmax(262px,1fr)_minmax(154px,174px)] gap-2.5">
            ...
            {undoState && (
                <div className="pointer-events-none fixed bottom-6 left-1/2 z-[95] -translate-x-1/2">
                    <div className="pointer-events-auto flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 shadow-[0_16px_35px_rgba(15,23,42,0.16)]">
                        <span>׳”׳›׳¨׳˜׳™׳¡ ׳”׳•׳¡׳¨ ׳׳”׳“׳©׳‘׳•׳¨׳“</span>
                        <button type="button" onClick={restoreRemovedKpi} className="rounded-xl bg-blue-50 px-3 py-1 text-xs font-black text-blue-700 transition hover:bg-blue-100">׳‘׳˜׳</button>
                        <button type="button" onClick={dismissUndo} className="rounded-xl px-2 py-1 text-xs font-black text-slate-400 transition hover:bg-slate-100 hover:text-slate-600">׳¡׳’׳•׳¨</button>
                    </div>
                </div>
            )}
        </div>
