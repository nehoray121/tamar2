...
import Icon from '../../../components/common/Icon.jsx';
        ...
        const DashboardKpiCard = ({ title, subtitle, value, icon, accent, mode = 'dashboard', actionIcon, actionLabel, onAction, isActionDisabled = false }) => {
            ...
            return (
                <article className={`group relative flex h-full flex-col rounded-[24px] border bg-white text-right ${palette.border} ${palette.shadow} ${isModal ? 'min-h-[168px] px-4 pb-4 pt-5' : 'h-[clamp(124px,13vh,132px)] min-h-[124px] px-3 pb-2 pt-2'} ${!isModal ? 'dashboard-card-motion' : ''}`}>
                    {onAction && (
                        <button
                            type="button"
                            onClick={onAction}
                    ...
                    <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1 pr-1">
                            <h4 className={`font-black text-slate-900 ${isModal ? 'text-[10px] leading-5' : 'text-[12px] leading-[16px]'}`}>{title}</h4>
                            {subtitle && <p className={`mt-0.5 text-slate-400 ${isModal ? 'text-[10px] leading-4' : 'text-[10px] leading-[13px]'}`}>{subtitle}</p>}
                        </div>
                    ...
                    <div className={`mt-auto ${isModal ? 'pt-5' : 'pt-1 pb-4'}`}>
                        <div className={`font-black tracking-tight ${palette.value} ${isModal ? 'text-[44px] leading-none' : 'text-[30px] leading-none'}`}>{value}</div>
                        {!isModal && <div className="mt-1.5 text-[10px] font-semibold text-slate-400">׳¢׳•׳“׳›׳ ׳¢׳›׳©׳™׳•</div>}
                    </div>
                </article>
            );
        };
