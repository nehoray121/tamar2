...
const UrgencyDonutChart = ({ data, onSegmentClick, isExpanded = false, hideCenter = false }) => {
    ...
    const chartSize = 204;
    const labelPadding = isExpanded ? 54 : 32;
    ...
    const center = size / 2;
    const chartOffsetY = isExpanded ? 0 : -8;
    let currentAngle = 0;
        ...
        <div className="relative flex items-center justify-center p-2">
            ...
            <svg width={size} height={size} className="relative z-10 overflow-visible drop-shadow-sm" style={{ transform: `translateY(${chartOffsetY}px)` }}>
                ...
                {data.map(item => {
                    ...
                    const connectorStart = pointOnCircle(labelAngle, outerRingRadius + (isExpanded ? 10 : 5));
                    const connectorBend = pointOnCircle(labelAngle, outerRingRadius + (isExpanded ? 34 : 16));
                    const isRightSide = connectorBend.x >= center;
                    const connectorEndX = connectorBend.x + (isRightSide ? (isExpanded ? 36 : 18) : (isExpanded ? -36 : -18));
                    const labelPoint = {
                        x: connectorEndX + (isRightSide ? (isExpanded ? 12 : 6) : (isExpanded ? -12 : -6)),
                        y: Math.min(size - (isExpanded ? 22 : 12), Math.max(isExpanded ? 22 : 12, connectorBend.y))
                    };
