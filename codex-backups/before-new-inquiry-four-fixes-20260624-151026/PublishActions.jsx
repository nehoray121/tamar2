...
const PublishActions = ({
    ...
    <div className="absolute bottom-5 left-8 z-30">
        <div className="relative flex h-14 items-center gap-2 rounded-2xl bg-white/95 p-2 shadow-[0_18px_36px_rgba(15,23,42,0.12)] ring-1 ring-blue-100 backdrop-blur">
            ...
            <button type="button" onClick={() => setIsShortcutsOpen((value) => !value)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-blue-100 bg-blue-50 text-blue-600">
                <Icon name="keyboard" className="h-5 w-5" />
            </button>

            <button type="button" onClick={() => setIsPublishConfirmOpen((value) => !value)} className="h-10 rounded-xl border border-blue-100 bg-white px-5 text-sm font-black text-slate-900 shadow-sm">
                פרסם וסגור
            </button>

            <button type="button" className="h-10 rounded-xl bg-blue-700 px-7 text-sm font-black text-white shadow-sm shadow-blue-200">
                פרסם פנייה
            </button>
        </div>
    </div>
