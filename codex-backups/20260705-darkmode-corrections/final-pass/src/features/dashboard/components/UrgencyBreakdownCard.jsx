        ...
        return (
            <DashboardCard className="dashboard-card-motion flex h-full min-h-0 flex-col p-3" dir="rtl">
                <div className="flex shrink-0 items-start justify-between gap-2">
                    <div>
                        <h2 className="text-[22px] font-black leading-7 text-slate-950">פילוח לפי דחיפות</h2>
    ...
    return (
        <DashboardCard className="dashboard-card-motion dashboard-expanded-card flex h-full min-h-0 flex-col p-3" dir="rtl">
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-2 pt-2">
                <div className="grid shrink-0 gap-2.5 xl:grid-cols-[minmax(270px,0.78fr)_minmax(0,1.22fr)]">
                    ...
                    <section className="flex min-h-[174px] flex-col rounded-[24px] border border-slate-200 bg-white shadow-sm">
                        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-3.5 py-2">
                            <h3 className="text-lg font-black text-slate-950">פילוח מדדים מפורט</h3>
                            <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-black text-slate-500">{donutCategories.length} קטגוריות</span>
                        </div>
                                    ...
                                    {visibleDonutCategoryCards.map((category, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="mt-1.5 text-xs font-semibold text-slate-500">{category.formattedPercentage}% מכלל הפניות</div>
                ...
                <section className={`flex min-h-0 flex-col overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm transition-[max-height,box-shadow,flex] duration-300 ease-in-out motion-reduce:transition-none ${showInlineDetails ? 'flex-1' : 'shrink-0'}`} style={showInlineDetails ? undefined : { maxHeight: `${collapsedPanelHeight}px` }}>
                    <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-4 py-2">
                        <div className="flex items-center gap-3">
                            <span className="text-lg font-black text-slate-950">פניות בקטגוריה שנבחרה</span>
                    ...
                    <div className="flex min-h-0 flex-1 flex-col px-3 py-2">
                        {visibleSelectedDonutInquiries.length === 0 ? (
                            ...
                            <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-slate-100 bg-slate-50/30">
                                ...
                                <div className={`min-h-0 flex-1 bg-white ${showInlineDetails ? 'overflow-y-auto' : 'overflow-hidden'}`}>
