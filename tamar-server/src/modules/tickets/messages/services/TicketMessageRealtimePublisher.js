    ...
    async publish(eventType, message, ticket) {
        ...
        try {
            ...
            const lineages = await Promise.all(
                visibleIds.map(async (roomId) => {
                    try {
                        return await this.organization
                            .integrityService.resolveRoom(
                                roomId,
                                {
            ...
            for (const lineage of lineages.filter(Boolean)) {
                rooms.add(
                    `environment:${lineage.environment._id}`
                );
                rooms.add(
                    `subEnvironment:${lineage.subEnvironment._id}`
                );
            ...
            this.logger?.warn(
                'ticket.message_realtime_publish_failed',
                {
                    eventType,
