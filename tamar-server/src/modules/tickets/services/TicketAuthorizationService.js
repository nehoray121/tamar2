...
class TicketAuthorizationService {
    ...
    resolveAccess(userId, options = {}) {
        return this.scopeResolver.resolveEffectiveAccess(userId, options);
    }
    ...
    canAccessCurrentRoom(access, ticket) {
        return Boolean(access?.isActive && (
            this.isSystemAdministrator(access, ticket)
    ...
    canAccessVisibleRooms(access, ticket) {
        ...
        return Boolean(access?.isActive && (
            this.isSystemAdministrator(access, ticket)
            || visibleRoomIds.some((roomId) => includesId(
                access.roomIds,
                roomId
            ))
        ));
    ...
    matchingMemberships(access, ticket) {
        return (access?.memberships || []).filter((membership) => (
            ...
            || (
                ...
                && sameId(membership.environmentId, ticket.environmentId)
            )
            || (
                membership.role === ROLES.SYSTEM_ADMIN
                && sameId(
                    membership.subEnvironmentId,
                    ticket.subEnvironmentId
                )
            )
            || (
                [ROLES.ROOM_MANAGER, ROLES.ROOM_USER].includes(
                    membership.role
                )
    ...
    canView(access, ticket) {
        return this.canAccessVisibleRooms(access, ticket);
    }
    ...
    canCreate(access, lineage) {
        return Boolean(access?.isActive && (
            this.isSystemAdministrator(access, {
    ...
    canAssign(access, ticket) {
        return this.hasAssignmentAuthority(access, ticket);
    }
    ...
    actorRoleContext(access, ticket) {
        return this.matchingMemberships(access, ticket)
            .sort((left, right) => (
                (roleRank[right.role] || 0) - (roleRank[left.role] || 0)
            ))[0]?.role || 'UNKNOWN';
    }
    ...
    buildAccessFilter(access, { historical = false } = {}) {
        ...
        const systemIds = (access.memberships || [])
            .filter((item) => item.role === ROLES.SUPER_ADMIN)
            .map((item) => String(item.systemId));

        const scopes = [];
        if (systemIds.length) {
            scopes.push({ systemId: { $in: [...new Set(systemIds)] } });
        }
        if ((access.roomIds || []).length) {
            scopes.push({
                [historical ? 'visibleRoomIds' : 'currentRoomId']: {
                    $in: access.roomIds
                }
            });
        }
