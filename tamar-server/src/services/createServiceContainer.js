...
const createServiceContainer = ({ config, logger } = {}) => {
    ...
    const hierarchyIntegrityService = new HierarchyIntegrityService({ hierarchyRepository: organization.hierarchyAdapter });
    ...
    const organizationHierarchyAuthorizationService = new OrganizationHierarchyAuthorizationService({
        scopeResolver,
        integrityService: organization.integrityService
    });
    const organizationHierarchyMutationService = new OrganizationHierarchyMutationService({
        managementService: organization.managementService,
        authorizationService: organizationHierarchyAuthorizationService
    });
    organization.authorizationService = organizationHierarchyAuthorizationService;
    organization.mutationService = organizationHierarchyMutationService;
    const approvalPolicy = new AccessRequestApprovalPolicy({ scopeResolver, hierarchyIntegrityService });
    let auth = null;
    let realtimePublisher = null;
    if (config?.auth) {
        const personalNumberService = new PersonalNumberService({ hmacKey: config.auth.identityLookupHmacKey, pattern: config.auth.personalNumberPattern });
        const accessTokenVerifier = new AccessTokenVerifier({ authConfig: config.auth });
        const claimsMapper = new IdentityClaimsMapper({ authConfig: config.auth, personalNumberService });
        const authenticatedIdentityService = new AuthenticatedIdentityService({ userRepository, accessRequestRepository, scopeResolver, personalNumberService });
        const accessRequestOptionsService = new AccessRequestOptionsService({ organization });
        const requestThrottle = new IdentityRequestThrottle();
        realtimePublisher = new AccessFlowRealtimePublisher({ personalNumberService });
        auth = { personalNumberService, accessTokenVerifier, claimsMapper, authenticatedIdentityService, accessRequestOptionsService, requestThrottle, logger };
    }
    const accessRequestService = new AccessRequestService({ userRepository, membershipRepository, accessRequestRepository, hierarchyIntegrityService, approvalPolicy, scopeResolver, realtimePublisher });
    const membershipService = new MembershipService({ userRepository, membershipRepository, hierarchyIntegrityService, authorizationService });
    const protectedRoleAssignmentService = new ProtectedRoleAssignmentService({ userRepository, membershipRepository, scopeResolver, hierarchyIntegrityService });
    const settingsService = new SettingsService({ organization, scopeResolver });
    const userManagementService = auth ? new UserManagementService({
        userRepository, membershipRepository, scopeResolver, membershipService,
        protectedRoleAssignmentService, personalNumberService: auth.personalNumberService
    }) : null;
    const analyticsService = userManagementService ? new AnalyticsService({ organization, scopeResolver, userManagementService }) : null;
    ...
    const ticketAssignmentRepository = new TicketAssignmentRepository();
    const ticketTransferRepository = new TicketTransferRepository();
    const ticketTransferTargetRepository = new TicketTransferTargetRepository();
    ...
    const ticketAuthorizationService = new TicketAuthorizationService({ scopeResolver });
    const ticketCapabilityService = new TicketCapabilityService({ authorizationService: ticketAuthorizationService });
    const ticketRealtimePublisher = new TicketRealtimePublisher({ logger });
    const assignmentRealtimePublisher = new TicketAssignmentRealtimePublisher({ logger });
    const transferRealtimePublisher = new TicketTransferRealtimePublisher({ logger });
    const messageRealtimePublisher = new TicketMessageRealtimePublisher({ organization, logger });
    const boardRealtimePublisher = new TicketBoardRealtimePublisher({ logger });
    const transferAuthorizationService = new TicketTransferAuthorizationService({
        ticketAuthorizationService
    });
    const messageAuthorizationService = new TicketMessageAuthorizationService({
        ticketAuthorizationService
    });
    const messageCapabilityService = new TicketMessageCapabilityService();
    const assigneeSummaryService = new TicketAssigneeSummaryService({ userRepository });
    const transactionRunner = new MongoTransactionRunner();
    const ticketService = new TicketService({
        organization, ticketRepository, historyRepository: ticketHistoryRepository,
        sequenceRepository: ticketSequenceRepository, authorizationService: ticketAuthorizationService,
        capabilityService: ticketCapabilityService, transactionRunner,
        realtimePublisher: ticketRealtimePublisher, assigneeSummaryService,
        transferRepository: ticketTransferRepository
    });
    const ticketAssignmentService = new TicketAssignmentService({
        organization, userRepository, membershipRepository, ticketRepository,
        assignmentRepository: ticketAssignmentRepository, historyRepository: ticketHistoryRepository,
        authorizationService: ticketAuthorizationService, capabilityService: ticketCapabilityService,
        transactionRunner, assigneeSummaryService,
        assignmentRealtimePublisher, ticketRealtimePublisher
    });
    const ticketTransferService = new TicketTransferService({
        organization, ticketRepository, assignmentRepository: ticketAssignmentRepository,
        historyRepository: ticketHistoryRepository, transferRepository: ticketTransferRepository,
        authorizationService: ticketAuthorizationService, capabilityService: ticketCapabilityService,
        transferAuthorizationService, transactionRunner, assigneeSummaryService,
        realtimePublisher: transferRealtimePublisher
    });
    const ticketTransferQueryService = new TicketTransferQueryService({
        organization, ticketRepository, transferRepository: ticketTransferRepository,
        authorizationService: ticketAuthorizationService, transferAuthorizationService,
        capabilityService: ticketCapabilityService, userSummaryService: assigneeSummaryService
    });
    const ticketTransferTargetService = new TicketTransferTargetService({
        ticketRepository, targetRepository: ticketTransferTargetRepository,
        authorizationService: ticketAuthorizationService
    });
    const ticketMessageService = new TicketMessageService({
        ticketRepository, messageRepository: ticketMessageRepository,
...
const boardAuthorizationService = new TicketBoardAuthorizationService({ organization, scopeResolver });
    ...
    const boardEligibilityService = new TicketBoardEligibilityService({
        ticketRepository, transferRepository: ticketTransferRepository
    });
    ...
    const assignmentController = new TicketAssignmentController({ assignmentService: ticketAssignmentService });
...
const boardController = new TicketBoardController({
    ...
    return {
        organization, userRepository, membershipRepository, accessRequestRepository, hierarchyIntegrityService,
        scopeResolver, authorizationService, approvalPolicy, accessRequestService, membershipService,
        protectedRoleAssignmentService, settingsService, userManagementService, analyticsService, realtimePublisher, auth,
        tickets: {
            ticketRepository, ticketHistoryRepository, ticketSequenceRepository, ticketAssignmentRepository,
            ticketTransferRepository, ticketTransferTargetRepository, ticketMessageRepository,
            boardCategoryRepository, boardStateRepository, boardQueryRepository,
            authorizationService: ticketAuthorizationService, capabilityService: ticketCapabilityService,
            transferAuthorizationService, messageAuthorizationService, messageCapabilityService,
            realtimePublisher: ticketRealtimePublisher, assignmentRealtimePublisher, transferRealtimePublisher,
            messageRealtimePublisher, boardRealtimePublisher,
            assigneeSummaryService, ticketService, assignmentService: ticketAssignmentService,
            transferService: ticketTransferService, transferQueryService: ticketTransferQueryService,
            transferTargetService: ticketTransferTargetService,
            messageService: ticketMessageService, messageQueryService: ticketMessageQueryService,
            boardAuthorizationService, boardCapabilityService, boardEligibilityService,
            boardCategoryService, boardItemStateService, boardQueryService,
            controller: ticketController, assignmentController, transferController, messageController, boardController
        }
    };
};
