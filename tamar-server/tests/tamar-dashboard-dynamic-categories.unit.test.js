...
test('dashboard dynamic category pipeline groups priority at top level and custom fields under fieldValues', () => {
    ...
    assert.deepEqual(AnalyticsService.dashboardCategoryPipeline(undefined, summary), [{ $match: { $expr: { $eq: [1, 0] } } }]);
});


test('empty category pipeline never emits an illegal zero limit', () => {
    const pipeline = AnalyticsService.dashboardCategoryPipeline(undefined, { ticketNumber: '$ticketNumber' });
    assert.deepEqual(pipeline, [{ $match: { $expr: { $eq: [1, 0] } } }]);
    assert.equal(JSON.stringify(pipeline).includes('\"$limit\":0'), false);
});
