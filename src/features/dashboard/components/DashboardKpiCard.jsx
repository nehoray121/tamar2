...
import Icon from '../../../components/common/Icon.jsx';
        ...
        const DashboardKpiCard = ({ title, subtitle, value, icon, accent, mode = 'dashboard', actionIcon, actionLabel, onAction, isActionDisabled = false }) => {
            ...
            return (
                <article className={`group relative flex h-full flex-col rounded-[20px] border bg-[var(--color-surface-raised)] text-right ${palette.border} ${palette.shadow} ${isModal ? 'min-h-[168px] px-4 pb-4 pt-5' : 'h-[clamp(106px,11vh,114px)] min-h-[106px] px-2.5 pb-1.5 pt-2'} ${!isModal ? 'dashboard-card-motion' : ''}`}>
                    {onAction && (
                        <button
                            type="button"
                            onClick={onAction}
                    ...
                    <div className={`mt-auto ${isModal ? 'pt-5' : 'pt-0.5 pb-2.5'}`}>
                        <div className={`font-black tracking-tight ${palette.value} ${isModal ? 'text-[44px] leading-none' : 'text-[26px] leading-none'}`}>{value}</div>
