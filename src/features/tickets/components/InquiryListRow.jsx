...
const RowMenu = ({ ticket, categories, onAssignCategory, onClose }) => {
    ...
    return (
        <div className="absolute left-0 top-full z-50 mt-1.5 w-56 overflow-hidden rounded-2xl border border-[#C9E1FF] bg-white p-1.5 text-right shadow-[0_18px_44px_rgba(37,99,235,0.18)]" dir="rtl">
            {onAssignCategory && (
                <>
...
const InquiryListRow = ({
    ...
    return (
        <article
            ...
            onDrop={onDrop}
            className={`group flex min-h-[56px] w-full min-w-0 flex-wrap items-center gap-x-4 gap-y-2 rounded-2xl border px-3 py-2 shadow-[0_6px_18px_rgba(37,99,235,0.10)] transition hover:-translate-y-0.5 hover:border-[#93C5FD] hover:bg-[#F8FBFF] hover:shadow-[0_12px_26px_rgba(37,99,235,0.14)] ${
                selected
                    ? 'border-[#60A5FA] bg-[#F8FBFF] ring-1 ring-blue-100 shadow-[0_10px_24px_rgba(37,99,235,0.16)]'
                    : 'border-[#C9E1FF] bg-white'
            }`}
        >
            {selectionMode && <SelectionCheckbox ticketId={ticket.id} selected={selected} onToggleSelection={onToggleSelection} />}
            ...
            <div className="grid min-w-[320px] flex-[1_1_520px] grid-cols-2 items-center gap-x-5 gap-y-2 lg:grid-cols-4">
            ...
            <div className="relative ms-auto flex shrink-0 items-center gap-2" dir="ltr">
                <button
                    type="button"
                    title="פעולות נוספות"
                    aria-label={`פעולות נוספות עבור ${ticket.id}`}
                    aria-expanded={menuOpen}
                    onClick={(event) => {
                        event.stopPropagation();
                        setMenuOpen((value) => !value);
                    }}
                    className="flex h-8 w-5 shrink-0 items-center justify-center rounded-lg bg-transparent text-[#3B82F6] transition hover:bg-[#EAF4FF] focus:outline-none focus:ring-2 focus:ring-blue-400/40"
                >
                ...
                {menuOpen && <RowMenu ticket={ticket} categories={categories} onAssignCategory={onAssignCategory} onClose={() => setMenuOpen(false)} />}
            </div>
        </article>
    );
};
