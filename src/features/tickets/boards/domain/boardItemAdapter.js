...
export const formatVersionEtag = (version) => (
    `"${Number(version) || 0}"`
);

export const adaptCategory = (category, etag = null) => (
    category
        ? {
            ...category,
...
export const adaptBoardState = (state, etag = null) => ({
    ...
    canChangeCategory: Boolean(
        state?.capabilities?.canChangeCategory
    ),
...
export const adaptBoardItem = (item) => {
    ...
    const transferId = external
        ? String(item?.transfer?.id || '')
        : null;
    const boardItemId = external ? transferId : ticketId;

    if (!ticketId || !boardItemId || (external && !transferId)) {
        throw new TypeError(
            'Board item is missing its canonical Ticket or Transfer identity'
        );
    }

    const state = adaptBoardState(item.boardState);
    const fieldValues = item.ticket?.fieldValues || {};
    const eventDate = external
        ? item.transfer?.initiatedAt
        : (item.ticket?.createdAt || item.ticket?.updatedAt);
    ...
    return {
        ...
        transferVersion: external
            && Number.isFinite(Number(item.transfer?.version))
            ? Number(item.transfer.version)
            : null,
        capabilities: item.ticket?.capabilities || {},
        displayId: item.ticket?.ticketNumber || ticketId,
        priority: priorityLabels[item.ticket?.priority]
            || 'בינונית-2',
        name: item.ticket?.subject || 'ללא נושא',
        room: external
            ? (
                boardType === BOARD_TYPES.EXTERNAL_SENT
                    ? item.transfer?.destinationRoomId
                    : item.transfer?.sourceRoomId
            )
        ...
        submissionStatus: (
            externalStatusLabels[item.transfer?.externalState] || null
        ),
        isOutgoingExternal: (
            boardType === BOARD_TYPES.EXTERNAL_SENT
        ),
...
export const replaceBoardItemState = (
    row,
    state,
    etag = null
) => {
