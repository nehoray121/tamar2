...
import { loadLocalAuthenticationUi } from '../../features/auth/runtimeAuthentication.js';
...
const navItemsBase = [
    { id: 'super_admin', icon: 'shield', label: 'מרכז שליטה', superAdminOnly: true },
    { id: 'access_requests', icon: 'shield', label: 'בקשות גישה', reviewerOnly: true },
...
const LoadingGate = () => (
    <div className="flex min-h-screen w-full items-center justify-center bg-slate-50 text-slate-500 dark:bg-slate-950 dark:text-slate-300" dir="rtl">
...
const ErrorGate = ({ message, onRetry }) => (
    <div className="flex min-h-screen w-full items-center justify-center bg-slate-50 px-5 dark:bg-slate-950" dir="rtl">
        <section className="w-full max-w-lg rounded-3xl border border-red-100 bg-white p-8 text-slate-900 shadow-xl dark:border-red-900/40 dark:bg-slate-900 dark:text-slate-50">
            ...
            <p className="mt-3 leading-7 text-slate-500 dark:text-slate-300">{message}</p>
            <button
                type="button"
                onClick={onRetry}
...
function AppShell() {
    ...
    const organizationHierarchy = useSessionStore((state) => state.organizationHierarchy);
    const authRuntimeMode = useSessionStore((state) => state.authRuntimeMode);
    ...
    const resetLocalSession = useSessionStore((state) => state.resetLocalSession);
    ...
    const returnFromSuperAdmin = useSessionStore((state) => state.returnFromSuperAdmin);
    ...
    const canManageUsers = memberships.some((membership) => (
        ['SUPER_ADMIN', 'SYSTEM_ADMIN', 'ROOM_MANAGER'].includes(membership.role)
    ));
    const navItems = navItemsBase.filter((item) => (
        (!item.superAdminOnly || isSuperAdmin)
        && (!item.reviewerOnly || capabilities.reviewAccessRequests)
    ));
    ...
    const localLoginVisible = authRuntimeMode === 'local-personal-number'
        && ['local-login-required', 'obtaining-token', 'expired'].includes(authStatus);
    ...
    if (authStatus !== 'authenticated') {
        return <ErrorGate message={authError || 'ההתחברות אינה זמינה.'} onRetry={() => initializeRuntimeContext({ force: true })} />;
    }

    const showSidebar = hasSelectedRoom && !['hierarchy', 'user_management', 'access_requests'].includes(currentView);
    ...
    return (
        <div className="inquiry-page-surface flex h-screen w-full overflow-hidden font-sans" dir="rtl">
            {showEnvModal && (
            ...
            {authRuntimeMode === 'local-personal-number' && LocalSessionResetButton && (
                <LocalSessionResetButton onReset={resetLocalSession} />
            )}
        </div>
    );
}
