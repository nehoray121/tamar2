        ...
        <div className="space-y-4">
            ...
            {mode === 'table' ? (
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
                    <SectionShell title="תצוגה מקדימה" className="overflow-hidden"><div className="p-4"><div className="overflow-hidden rounded-2xl border border-[#D7E7FF]"><div className="grid bg-slate-50 px-4 py-3 text-xs font-black text-slate-400" style={{ gridTemplateColumns: `repeat(${tableFields.length || 1}, minmax(0, 1fr))` }}>{tableFields.map((id) => <span key={id}>{byId.get(id)?.name}</span>)}</div><div className="grid px-4 py-4 text-sm font-bold text-slate-700" style={{ gridTemplateColumns: `repeat(${tableFields.length || 1}, minmax(0, 1fr))` }}>{tableFields.map((id) => <span key={id}>{byId.get(id)?.placeholder || '[ערך]'}</span>)}</div></div></div></SectionShell>
                    <SectionShell title="שדות בטבלה" meta={`${tableFields.length}/5`}><div className="space-y-2 p-3">{tableFields.map((id, index) => <div key={id} className="flex items-center gap-2 rounded-xl border border-[#D7E7FF] bg-white px-3 py-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#EAF4FF] text-xs font-black text-[#2563EB]">{index + 1}</span><span className="flex-1 text-sm font-black text-slate-800">{byId.get(id)?.name}</span><button type="button" onClick={() => setTableFields((current) => current.filter((item) => item !== id))} className="text-slate-300 hover:text-red-500"><Icon name="trash" className="h-4 w-4" /></button></div>)}{tableFields.length < 5 && <select defaultValue="" onChange={(event) => { if (event.target.value) setTableFields((current) => [...current, event.target.value]); event.target.value = ''; }} className="h-10 w-full rounded-xl border border-dashed border-[#BBD7FF] bg-white px-3 text-sm font-bold text-[#2563EB]"><option value="">הוסף שדה לטבלה</option>{availableFields.map((field) => <option key={field.id} value={field.id}>{field.name}</option>)}</select>}</div></SectionShell>
                </div>
            ) : (
