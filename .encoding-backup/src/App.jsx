                    ...
                    <div className="grid grid-cols-3 gap-6 max-w-4xl mx-auto">
                        {envs.map(env => (
                            <div
                                ...
                                className={`bg-white border rounded-2xl p-5 cursor-pointer transition-all shadow-sm flex flex-col items-center justify-center min-h-[120px] gap-3 relative ${selectedId === env.id ? 'border-[#1E4DB7] ring-1 ring-[#1E4DB7] bg-blue-50/30' : 'border-gray-200 hover:border-gray-300 hover:shadow-md'
                                    }`}
                            >
...
const DashboardSegmentedButton = ({ label, isActive, onClick }) => (
    <button
        type="button"
        onClick={onClick}
        className={`rounded-xl px-4 py-2 text-sm font-black transition-all active:scale-95 ${isActive
        ...
        <article className={`relative flex h-full flex-col rounded-[24px] border bg-white text-right ${palette.border} ${palette.shadow} ${isModal ? 'min-h-[168px] px-4 pb-4 pt-5' : 'min-h-[170px] px-5 pb-5 pt-5'}`}>
            {onAction && (
                <button
                    ...
                    className={`absolute left-3 top-3 inline-flex h-7 w-7 items-center justify-center rounded-full border border-transparent transition focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-300 disabled:cursor-not-allowed disabled:opacity-35 ${actionIcon === 'trash'
...
const CustomBarChart = ({ data, onBarClick, barsPerPage = 6, isExpanded = false }) => {
    ...
    if (data.length === 0) {
        return (
            <div className="flex h-full min-h-[220px] flex-col items-center justify-center gap-4 rounded-b-[28px] bg-gradient-to-b from-[#F8FBFF] to-[#EEF5FF] text-slate-400">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex min-h-0 h-full items-end justify-center">
                                                <span
                                                    className={`w-full max-w-[58px] rounded-t-[18px] transition-all duration-300 group-hover:brightness-95 group-hover:drop-shadow-xl ${isHot ? 'bg-gradient-to-b from-pink-400 via-pink-200 to-blue-50/0' : 'bg-gradient-to-b from-blue-600 via-blue-300 to-blue-50/0'
                                                        }`}
                    ...
                    <div className="flex gap-6 mt-6 border-b border-gray-200">
                        {availableTabs.map(tab => (
                            <button
                                ...
                                className={`pb-3 px-1 text-sm font-bold transition-all relative ${activeTab === tab.id
                                        ? 'text-[#1E4DB7]'
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all mb-1.5 border ${isActive
                                            ? 'bg-[#EFF6FF] border-[#1E4DB7] shadow-sm'
                                            : 'bg-white border-transparent hover:bg-gray-50 hover:border-gray-200'
                                        }`}
                                >
                                    <div className="flex items-center gap-2.5">
                                        <div className={`p-1.5 rounded-lg flex items-center justify-center transition-colors shadow-sm ${isActive ? 'bg-[#1E3A8A] text-white' : 'bg-[#EFF6FF] text-[#1E4DB7]'
                                            }`}>
