        ...
        const DashboardInquiryListItem = ({ item }) => (
            <article className="group mb-3 flex w-full items-center justify-between gap-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-raised)] p-3 shadow-sm transition-all hover:border-blue-500/35 hover:shadow-md">
                <div className="flex shrink-0 items-center gap-2">
                    ...
                    <button type="button" className="inquiry-control rounded-xl p-2 text-[var(--color-primary)] transition-colors">
                ...
                <div className="min-w-0 flex-1">
                    <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
                        <div className="flex min-w-0 items-center gap-2 font-black text-[var(--color-text-primary)]">
                            ...
                            <DashboardBadge
                                className="dashboard-accent-badge"
                                style={{ '--dashboard-badge-accent': item.chartColor || 'var(--color-primary)' }}
                            >
                                {item.priority}
                            </DashboardBadge>
                            <DashboardBadge className={`dashboard-status-badge ${item.status === 'open' ? 'dashboard-status-badge--open' : 'dashboard-status-badge--closed'}`}>
