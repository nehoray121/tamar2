...
const UrgencyDonutChart = ({
    ...
    if (segmentTotal === 0) {
        const emptySize = isExpanded ? 250 : 154;
        return (
            <div className="tamar-v22-donut-chart tamar-v22-donut-chart--empty relative flex h-full min-h-[140px] w-full items-center justify-center">
                <div
                    className="tamar-v22-empty-donut-ring"
                    style={{ width: emptySize, height: emptySize }}
                    aria-hidden="true"
                />
                {!hideCenter && (
                    <div className="tamar-v22-empty-donut-center absolute z-10 flex flex-col items-center justify-center">
                        <span>0</span>
                        <small>סה״כ פניות</small>
                    </div>
                )}
            </div>
        );
    ...
    return (
        <div className="tamar-v22-donut-chart relative flex items-center justify-center p-1.5">
            ...
            {!hideCenter && (
                <div
                    className={`${isExpanded ? 'px-7 py-6' : 'px-4 py-3.5'} donut-center absolute z-20 flex cursor-default flex-col items-center justify-center rounded-full bg-[var(--color-surface-raised)]`}
                    style={{ transform: `translateY(${chartOffsetY}px)` }}
                >
                    <span className={`${isExpanded ? 'text-[34px]' : 'text-[25px]'} font-black`} style={{ color: 'var(--chart-label)' }}>
                        {total}
                    </span>
                    <span className={`${isExpanded ? 'text-[13px]' : 'text-[11px]'} font-bold`} style={{ color: 'var(--chart-label-secondary)' }}>
                        סה״כ פניות
                    </span>
                </div>
            )}
