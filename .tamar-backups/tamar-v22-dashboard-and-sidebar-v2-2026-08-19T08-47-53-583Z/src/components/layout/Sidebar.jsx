...
const SidebarNavItem = ({
    ...
    <button
        ...
        onClick={onClick}
        className={`tamar-reference-sidebar-item flex w-full items-center justify-between text-sm transition-colors duration-150 focus-visible:outline-none ${isActive ? 'tamar-reference-sidebar-item--active' : ''}`}
        style={{
    ...
    >
        ...
        {badge !== undefined && badge !== null && (
            <span className="tamar-reference-sidebar-badge inline-flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-[10px] font-bold">
                {badge}
            </span>
        )}
    </button>
...
const SidebarBrand = ({ onNavigate }) => (
    <div className="tamar-reference-sidebar-brand mx-6 flex h-24 shrink-0 items-center justify-center gap-2 border-b border-[var(--color-border)]">
        <Icon
            name="layers"
...
const SidebarFooter = ({ currentUser }) => {
    ...
    return (
        <div className="tamar-reference-sidebar-footer relative mt-auto shrink-0 border-t border-[var(--color-border)] px-6 py-5 transition-colors hover:bg-[var(--color-surface-muted)]">
            <ThemeControl />
...
const Sidebar = ({
    ...
    <aside className="tamar-reference-sidebar fixed inset-y-0 right-0 z-30 flex h-screen w-64 flex-col border-l border-[var(--color-border)] bg-[var(--color-surface-raised)] text-[var(--color-text-primary)] shadow-[-8px_0_30px_rgba(0,0,0,0.025)]">
        <SidebarBrand
