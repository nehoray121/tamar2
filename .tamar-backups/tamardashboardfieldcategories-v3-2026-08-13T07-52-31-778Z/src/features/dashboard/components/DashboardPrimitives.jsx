...
import Icon from '../../../components/common/Icon.jsx';
        ...
        const DashboardToolbarPill = ({ children, className = '' }) => (
            <div className={`inquiry-control flex items-center gap-2 rounded-2xl px-3 py-2 shadow-sm transition-all focus-within:border-blue-300 focus-within:ring-4 focus-within:ring-blue-100/70 dark:focus-within:border-blue-500 dark:focus-within:ring-blue-500/30 ${className}`}>
                {children}
            </div>
        );
        ...
        const DashboardSelectPill = ({ label, icon, value, onChange, options }) => (
            <DashboardToolbarPill>
                ...
                <select
                    ...
                    className="min-w-[132px] cursor-pointer border-0 bg-transparent text-sm font-black inquiry-primary-text outline-none"
                >
        ...
        const DashboardDateInput = ({ label, value, onChange }) => (
            <DashboardToolbarPill>
                ...
                <input
                    ...
                    onChange={(event) => onChange(event.target.value)}
                    className="cursor-pointer border-0 bg-transparent text-sm font-black inquiry-primary-text outline-none"
                />
            </DashboardToolbarPill>
        );
