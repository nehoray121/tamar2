...
const UsersPermissionsTab = ({ data, onRefresh }) => {
    ...
    const filteredRows = useMemo(() => data.users.rows.filter((user) => {
        const matchesQuery = user.name.includes(query.trim()) || user.id.includes(query.trim());
        const matchesRole = roleFilter === 'all' || user.primaryRole === roleFilter;
        const matchesStatus = statusFilter === 'all' || user.status === statusFilter;
        return matchesQuery && matchesRole && matchesStatus;
    }), [data.users.rows, query, roleFilter, statusFilter]);

    const pageRows = filteredRows.slice(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE);
    const rows = pageRows.map((user) => ({
        id: user.id,
        raw: user,
        cells: [user.name, user.personalNumberMasked || user.id, user.roleLabel, user.scopeLabel, user.status, user.lastActivity, user.anomalies ? `${user.anomalies} חריגות` : '-']
    }));
    ...
    return (
        <div className="flex h-full min-h-0 flex-col gap-3 overflow-hidden">
            <KpiStrip items={data.users.kpis.map((item) => ({ ...item, id: item.title, subtitle: 'בטווח הבחירה', icon: item.tone === 'danger' ? 'alertTriangle' : 'users' }))} />
            <SuperAdminCard className="flex min-h-0 flex-1 flex-col overflow-hidden">
                <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-[var(--color-border)] p-3">
                    <div className="flex flex-wrap gap-2">
                        <input value={query} onChange={(event) => { setQuery(event.target.value); setPage(0); }} className="inquiry-input-surface h-9 w-52 rounded-lg px-3 text-[12px] font-bold outline-none" placeholder="חיפוש משתמש..." />
                        <select value={roleFilter} onChange={(event) => { setRoleFilter(event.target.value); setPage(0); }} className="inquiry-input-surface h-9 rounded-lg px-3 text-[12px] font-bold">
                            <option value="all">כל התפקידים</option>
                            {capabilities.allowedRoles.map((role) => <option key={role.id} value={role.id}>{role.label}</option>)}
                        </select>
                        <select value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setPage(0); }} className="inquiry-input-surface h-9 rounded-lg px-3 text-[12px] font-bold"><option value="all">כל הסטטוסים</option><option value="פעיל">פעיל</option><option value="לא פעיל">לא פעיל</option></select>
                    </div>
                    <button type="button" onClick={() => setDrawerItem({ mode: 'create' })} className="h-9 rounded-lg bg-[var(--color-primary)] px-3 text-[12px] font-black text-white">הוספת משתמש +</button>
                </div>
                <div className="min-h-0 flex-1 p-3">
                    <CompactTable columns={['משתמש', 'מזהה', 'תפקיד ראשי', 'תחום ארגוני', 'סטטוס', 'פעילות אחרונה', 'חריגות']} rows={rows} onRowClick={(row) => setDrawerItem({ mode: 'manage', ...row.raw })} />
                </div>
            ...
            <Drawer
                title={drawerItem?.mode === 'create' ? 'יצירת משתמש חדש' : managedUser?.name || drawerItem?.name || 'ניהול משתמש'}
                subtitle={drawerItem?.mode === 'create' ? 'השתמשו בטופס היצירה הקיים של המערכת.' : 'פרטי משתמש, הרשאות, שיוכים והיסטוריית פעולות.'}
                item={drawerItem}
            ...
            >
                {drawerItem?.mode === 'create' ? (
                    ...
                    <div className="space-y-3 text-[13px] font-semibold leading-6 text-[var(--color-text-secondary)]">
                        ...
                        <p>המשתמש אינו מנוהל עדיין במסך ההרשאות המלא, אך פרטי הסיכום שלו נשמרו בטבלה.</p>
                        <p>תפקיד: {drawerItem?.roleLabel}</p>
                        <p>תחום: {drawerItem?.scopeLabel}</p>
                        <p>פעילות אחרונה: {drawerItem?.lastActivity}</p>
                    </div>
                )}
            </Drawer>
        </div>
