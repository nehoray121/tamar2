...
const sortDashboardItems = (items) => [...items].sort((left, right) => {
    const urgentOrder = Number(isUrgentInquiry(right))
        - Number(isUrgentInquiry(left));
...
export const filterDashboardInquiriesForKpi = (
    ...
    const predicate = {
        ...
        urgent: (item) => isOpenInquiry(item) && isUrgentInquiry(item),
...
export const buildDashboardWorkloadPeople = (
    ...
    (Array.isArray(workloadRows) ? workloadRows : []).forEach((row, index) => {
        ...
        const explicitUrgent = Number(row?.urgent);
        ...
        people.push({
            ...
            total: Number.isFinite(explicitTotal)
                ...
                : tasks.length,
            urgent: Number.isFinite(explicitUrgent)
                ? Math.max(0, explicitUrgent)
                : tasks.filter(isUrgentInquiry).length,
            tasks
        });
    });
    ...
    missingPeople.forEach((person) => {
        ...
        people.push({
            ...person,
            total: tasks.length,
            urgent: tasks.filter(isUrgentInquiry).length,
            tasks
        });
    });
