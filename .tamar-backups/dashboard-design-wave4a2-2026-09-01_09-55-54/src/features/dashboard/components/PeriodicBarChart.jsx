...
const PeriodicBarChart = ({ data, onBarClick, barsPerPage = 6, isExpanded = false, resetKey = '' }) => {
    ...
    if (data.length === 0) {
        const emptyHeights = [34, 48, 62, 82, 68, 44];
        return (
            <div className="tamar-v22-bar-chart tamar-v22-empty-chart relative flex h-full min-h-0 w-full flex-col overflow-hidden bg-[var(--color-surface-raised)] px-5 pb-4 pt-2">
                <div className="tamar-v22-empty-chart-message">אין נתונים להצגה</div>
                <div className="tamar-v22-empty-chart-bars" aria-hidden="true">
                    {emptyHeights.map((height, index) => (
                        <div key={height} className="tamar-v22-empty-chart-column">
                            <span
                                className={`tamar-v22-empty-chart-bar ${index === 3 ? 'tamar-v22-empty-chart-bar--accent' : ''}`}
                                style={{ height: `${height}%` }}
                            />
                            <span className="tamar-v22-empty-chart-label">—</span>
                        </div>
                    ))}
                </div>
            </div>
