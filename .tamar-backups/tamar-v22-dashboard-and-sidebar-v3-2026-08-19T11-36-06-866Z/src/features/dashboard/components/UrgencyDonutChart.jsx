...
const UrgencyDonutChart = ({ data, onSegmentClick, isExpanded = false, hideCenter = false, totalOverride }) => {
    ...
    if (segmentTotal === 0) {
        return (
            <div className="flex h-full min-h-[220px] w-full items-center justify-center bg-[var(--color-surface-raised)] text-sm font-bold text-[var(--color-text-muted)]">
                אין נתונים להצגה
            </div>
        );
    }
    ...
    return (
        <div className="tamar-v22-donut-chart tamar-reference-donut-chart relative flex items-center justify-center p-1.5">
            <div className={(isExpanded ? 'h-40 w-40' : 'h-32 w-32') + ' donut-halo pointer-events-none absolute rounded-full bg-[var(--color-surface-muted)] shadow-inner ring-1 ring-[var(--color-border)]'} style={{ transform: 'translateY(' + chartOffsetY + 'px)' }} />
