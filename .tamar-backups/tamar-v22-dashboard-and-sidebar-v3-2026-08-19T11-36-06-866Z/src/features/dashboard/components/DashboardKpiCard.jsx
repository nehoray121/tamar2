...
import Icon from '../../../components/common/Icon.jsx';
        ...
        const DashboardKpiCard = ({ title, subtitle, value, icon, accent, mode = 'dashboard', actionIcon, actionLabel, onAction, isActionDisabled = false }) => {
            ...
            return (
                <article data-accent={accent} data-mode={mode} className={`tamar-v22-kpi-card tamar-reference-kpi-card group relative flex flex-col rounded-[20px] border bg-[var(--color-surface-raised)] text-right ${palette.border} ${palette.shadow} ${isModal ? 'h-auto min-h-[148px] px-4 pb-3 pt-4' : 'h-full h-[clamp(106px,11vh,114px)] min-h-[106px] px-2.5 pb-1.5 pt-2'} ${!isModal ? 'dashboard-card-motion' : ''}`}>
                    {onAction && (
                        <button
                            type="button"
                            onClick={onAction}
